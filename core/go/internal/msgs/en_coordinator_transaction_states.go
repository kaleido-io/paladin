...
var (
	...
	MsgCoordinatorTxStatePreparing              = pdm("coordinator.transaction.State_Preparing", "Dispatch confirmation has been received from the originator and the transaction is being prepared and its dispatch built, with bounded retries, off the coordinator event loop")
	MsgCoordinatorTxStateReadyForDispatch       = pdm("coordinator.transaction.State_Ready_For_Dispatch", "The transaction's dispatch has been built and queued, and is waiting to be collected by the dispatch goroutine")
