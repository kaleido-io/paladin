...
var (
	...
	MsgStateViewQueryFailed            = pde("PD010138", "Remote state view query failed")
	MsgStateQueriedStateNoMatch        = pde("PD010139", "Coordinator returned state %s whose validated labels do not match the query")
	MsgStateQueriedStateSchemaMismatch = pde("PD010140", "Coordinator returned state %s with schema %s for a query against schema %s")
	MsgStateViewSpentIDsFailed         = pde("PD010141", "Failed to fetch spent state IDs from the remote state view")
