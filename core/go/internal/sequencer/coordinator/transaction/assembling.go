...
// index-aligned slices are derived from the settled PostAssembly: each resolved output state, its
// proto labels (StatesToStage holds the outputs first, in OutputStates order), and its distribution
// list from the matching potential output (the authoritative AllowedNodes source). The tracker stamps
// each state's created time itself.
// All three are 1:1 with OutputStates by construction of ResolvePotentialStates.
// This pattern of multiple aligned slices is arguably fragile, but the fragility is kept to the single
...
func (t *coordinatorTransaction) recordOutputStateVisibility(ctx context.Context) {
	...
	for i := range outputs {
		labels[i] = pa.StatesToStage[i].ProtoLabels()
...
// action_OpenStateViewSession initialises the assemble_request_id for this attempt and opens a
...
// flight. The assembleSessionIDis reused as the idempotency key of the assemble request.
