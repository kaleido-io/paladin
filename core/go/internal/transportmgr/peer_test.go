...
func TestQueryPeers(t *testing.T) {
	...
	tm := &transportManager{
		peers: map[string]*peer{
			"node1": {PeerInfo: pldapi.PeerInfo{Name: "node1", OutboundTransport: "grpc"}},
			"node2": {PeerInfo: pldapi.PeerInfo{Name: "node2"}},
			"node3": {PeerInfo: pldapi.PeerInfo{Name: "node3", OutboundTransport: "grpc"}},
		},
	}

	peers, err := tm.QueryPeers(ctx, query.NewQueryBuilder().
		Equal("outboundTransport", "grpc").
		Limit(10).
		Query())
	require.NoError(t, err)
