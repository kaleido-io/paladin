...
func createEthereumSignature(payloadBytes []byte) []byte {
	ethSig := &secp256k1.SignatureData{
		V: new(big.Int),
		R: new(big.Int),
		S: new(big.Int),
	}
	ethSig.V = ethSig.V.SetInt64(int64(payloadBytes[0]))
	ethSig.R = ethSig.R.SetBytes(payloadBytes[1:33])
	ethSig.S = ethSig.S.SetBytes(payloadBytes[33:65])

	return ethSig.CompactRSV()
}
...
func TestResolveKeyOk(t *testing.T) {
	ctx, httpEndpoint, done := newTestServer(t, func(r *http.Request) (int, interface{}) {
		...
		// Verify autoKeyCreation and KeySpec are not set when not specified in config
		assert.False(t, resolveReq.AutoKeyCreation)
		assert.Equal(t, "", resolveReq.KeySpec)
...
func TestResolveKeyWithAutoKeyCreation(t *testing.T) {
	...
	signingModule := newTestSigningModule(t)
	configureTestSigningModule(t, ctx, signingModule, httpEndpoint, `"autoKeyCreation": true`)
...
func TestResolveKeyWithoutAutoKeyCreation(t *testing.T) {
	ctx, httpEndpoint, done := newTestServer(t, func(r *http.Request) (int, interface{}) {
		resolveReq := validateResolveKeyRequest(t, r)

		// Verify autoKeyCreation and KeySpec are not set when autoKeyCreation is false
		assert.False(t, resolveReq.AutoKeyCreation)
		assert.Equal(t, "", resolveReq.KeySpec)
	...
	signingModule := newTestSigningModule(t)
	configureTestSigningModule(t, ctx, signingModule, httpEndpoint, `"autoKeyCreation": false`)
...
func TestResolveKeyKA058002ErrorWithoutAutoKeyCreation(t *testing.T) {
	ctx, httpEndpoint, done := newTestServer(t, func(r *http.Request) (int, interface{}) {
		validateResolveKeyRequest(t, r)
		// Return KA058002 error but without autoKeyCreation enabled
		return 400, createErrorResponse("KA058002: Key doesn't have a public identifier of type address_ethereum")
	})
	defer done()
	...
	require.Regexp(t, "KA180010", err) // Should still return KMS API call failed error
	require.Regexp(t, "KA058002", err) // Should include the original error message
}
...
func TestSignOk(t *testing.T) {
	payloadResp := "some signature data"

	ctx, httpEndpoint, done := newTestServer(t, func(r *http.Request) (int, interface{}) {
		validateSignRequest(t, r)
		return 200, createSignResponse(payloadResp)
	})
	defer done()
	...
	res, err := signingModule.Sign(ctx, &prototk.SignWithKeyRequest{
		KeyHandle:   testKeyID,
	...
	assert.Equal(t, payloadResp, string(res.Payload))
}
...
func TestSignOkECDSA_SECP256K1(t *testing.T) {
	...
	payloadResp, err := base64.StdEncoding.DecodeString(payloadRespBase64)
	require.NoError(t, err)
	...
	expectedSignature := createEthereumSignature(payloadResp)
	assert.Equal(t, expectedSignature, res.Payload)
}
...
func TestSignNonECDSAAlgorithm(t *testing.T) {
	ctx, httpEndpoint, done := newTestServer(t, func(r *http.Request) (int, interface{}) {
		validateSignRequest(t, r)
		return 200, createSignResponse("0x1234567890abcdef")
	})
	defer done()
	...
	require.NoError(t, err)
	assert.Equal(t, []byte("0x1234567890abcdef"), res.Payload)
}
