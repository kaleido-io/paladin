...
import { getEventListener } from "../queries/transactions";
import { EventListenerActions } from "../components/EventListenerActions";

export const EventListenerEntry: React.FC = () => {
  ...
  const { data: listener, error, refetch } = useQuery({
    queryKey: ['event-listener', id],
    queryFn: () => getEventListener(id!),
      ...
      >
        <Box sx={{ marginBottom: '20px' }}>
          <Button
            ...
            onClick={() => navigate(AppRoutes.EventListeners)}
          >
            {t('backToListeners')}
          </Button>
        </Box>
        <Typography variant="h6" sx={{ marginBottom: '15px' }}>{t('eventListener')}</Typography>
        <Tabs value="contract"
        ...
        <Box sx={{
          ...
          <EventListenerActions
            ...
            refetch={refetch}
            deleteRefetch={() => navigate(AppRoutes.EventListeners)}
          />
        </Box>
